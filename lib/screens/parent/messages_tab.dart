                ...
                child: Column(
                  ...
                  children: [
                    ...
                    if (item.attachmentCount > 0) ...[
                      ...
                      _Attachments(announcementId: item.id, count: item.attachmentCount),
                    ],
                  ],
                ),
              ),
...
///
/// Loaded when the notice is opened rather than with the list, because the list
/// already carries the COUNT — which is all that is needed to decide whether to
/// draw this at all — and fetching every attachment of every announcement on
/// every open would be a great deal of work for the few that are ever tapped.
class _Attachments extends StatefulWidget {
  const _Attachments({required this.announcementId, required this.count});
...
class _AttachmentsState extends State<_Attachments> {
  ...
  Future<void> _load() async {
    try {
      final files = await ParentApi.instance.announcementAttachments(widget.announcementId);
  ...
  Future<void> _open(AnnouncementFile f) async {
    final url = f.url;
  ...
  IconData _iconFor(AnnouncementFile f) {
